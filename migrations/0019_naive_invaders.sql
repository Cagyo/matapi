...
CREATE TABLE `__new_feature_install_jobs` (
	...
	`restart_scope` text,
	`restart_dispatch_identity` text,
	`failure_code` text,
	`created_at` integer NOT NULL,
	`updated_at` integer NOT NULL,
	FOREIGN KEY (`feature_name`) REFERENCES `features`(`name`) ON UPDATE no action ON DELETE no action,
	CONSTRAINT "feature_install_jobs_status_check" CHECK("__new_feature_install_jobs"."status" in ('queued', 'running', 'awaiting-restart', 'succeeded', 'failed')),
	CONSTRAINT "feature_install_jobs_operation_check" CHECK("__new_feature_install_jobs"."operation" in ('install', 'reinstall')),
	CONSTRAINT "feature_install_jobs_active_slot_check" CHECK((
      ("__new_feature_install_jobs"."status" in ('queued', 'running', 'awaiting-restart') and "__new_feature_install_jobs"."active_slot" is 1)
...
INSERT INTO `__new_feature_install_jobs`("id", "feature_name", "status", "active_slot", "operation", "requested_by_user_id", "requested_in_chat_id", "workflow_receipt_id", "previous_installed", "previous_enabled", "restart_scope", "restart_dispatch_identity", "failure_code", "created_at", "updated_at") SELECT "id", "feature_name", "status", "active_slot", "operation", "requested_by_user_id", "requested_in_chat_id", "workflow_receipt_id", "previous_installed", "previous_enabled", "restart_scope", "restart_dispatch_identity", "failure_code", "created_at", "updated_at" FROM `feature_install_jobs`;--> statement-breakpoint
