...
describe("createSetupServer", () => {
  it("rejects an unpaired token validation without calling validation", async () => {
    ...
    const response = await post(url, "/api/validate-token", {
      token: "bot-token",
    });
  ...
  it("does not write config when a paired finalization token is invalid", async () => {
    const { server, url, writeConfig } = await startTestServer({
      validateToken: vi
        .fn()
        .mockResolvedValue({ ok: false, error: "Invalid Telegram bot token." }),
    });
  ...
  it("reports selected RTSP as deferred without seeding env or feature authority", async () => {
    ...
    expect(features).toMatchObject({ enabled: [] });
    expect(features).not.toHaveProperty("liveStream");
    expect(await response.text()).toContain(
      "Finish Live view setup in Telegram",
    );
    await close(server);
  });

  it("keeps ordinary feature selections without RTSP deferral guidance", async () => {
    const installDir = createInstallDir();
    writeFileSync(
      join(installDir, ".env.example"),
