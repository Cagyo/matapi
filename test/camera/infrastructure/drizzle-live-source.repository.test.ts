...
describe('DrizzleLiveSourceRepository', () => {
  ...
  it('reads stored revisions and verification metadata back credential-free', async () => {
    const credentials = new AesGcmLiveSourceCredentialAdapter({ currentKey: oldKey, currentVersion: 1 });
    const repository = new DrizzleLiveSourceRepository(db, credentials);
    await repository.rotate();
    const source = LiveSource.create({
    ...
    // A fresh source is unversioned and unverified until the configuration
    // transaction that owns those columns writes them.
    expect(await repository.findRedacted('camera-1')).toEqual({
      cameraId: 'camera-1',
  ...
  it('reports a credential-free source and an absent one', async () => {
    const credentials = new AesGcmLiveSourceCredentialAdapter({ currentKey: oldKey, currentVersion: 1 });
    const repository = new DrizzleLiveSourceRepository(db, credentials);
    const source = LiveSource.create({ cameraId: 'camera-1', url: 'rtsp://cam.local/live', ready: false });
    await repository.save(source, null);

    expect(await repository.findRedacted('camera-1')).toMatchObject({ hasCredential: false, revision: 0 });
