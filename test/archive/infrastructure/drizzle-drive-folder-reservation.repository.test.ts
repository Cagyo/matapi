...
describe('DrizzleDriveFolderReservationRepository', () => {
  ...
  it('is idempotent by the globally unique folder ID', async () => {
    ...
    expect(sqlite.prepare(
      'SELECT id, generation_id AS generationId, normalized_path AS normalizedPath, folder_id AS folderId, state, current_slot AS currentSlot FROM drive_motion_folder_reservations',
    ).all()).toEqual([{
      id: 'missing-1', generationId: 'generation-1', normalizedPath: '2026',
      folderId: 'trashed-folder', state: 'missing', currentSlot: null,
    }]);
  });
  ...
  it('returns one stored and one exists for concurrent appends of one folder ID', async () => {
    ...
    expect(history()).toHaveLength(1);
    expect(sqlite.prepare(
      'SELECT folder_id AS folderId, state, current_slot AS currentSlot FROM drive_motion_folder_reservations',
    ).all()).toEqual([{ folderId: 'same-folder', state: 'missing', currentSlot: null }]);
  });
