...
describe("feature-management deployment boundary", () => {
  it("deploys all root bundle assets atomically with usable spool permissions", () => {
    for (const asset of [
      ...
      "install-feature.sh",
      "live-stream-net-helper",
      "live-stream-ffmpeg-runner",
      "live-view-policy-applier.py",
      "homeworker-feature-install.service",
    ...
    expect(install).toContain(
      "feature-install-requests /var/lib/home-worker/feature-install-results",
    );
    expect(install).toContain('-m 0770 -o root -g "$USER"');
    expect(install).toContain(
      "-m 0700 -o root -g root /var/lib/home-worker/feature-install-claims",
    );
    expect(install).toContain(
      "sudo install -d -m 0711 -o root -g root /var/lib/home-worker",
    );
    expect(install).toContain(
      '-m 0770 -o root -g "$USER" /var/lib/home-worker/live-view-settings-requests',
    );
    expect(install).toContain(
      "-m 0700 -o root -g root /var/lib/home-worker/live-view-settings-claims",
    );
    expect(install).toContain(
      '-m 0750 -o root -g "$USER" /var/lib/home-worker/live-view-settings-results',
    );
    expect(install).toContain(
  ...
  it("installs and proves the route inspection prerequisite before publishing the root bundle", () => {
    ...
    expect(
      mainBody.indexOf("install_feature_management_artifacts"),
    ).toBeGreaterThan(mainBody.indexOf("install_system_deps"));

    const body =
      /install_feature_management_artifacts\(\) \{([\s\S]*?)\n\}/.exec(
        install,
      )?.[1] ?? "";
    ...
    const guard =
      /require_route_inspection_prerequisite\(\) \{([\s\S]*?)\n\}/.exec(
        install,
      )?.[1] ?? "";
  ...
  it("publishes the live-view applier and atomically installs both policy unit copies", () => {
    expect(install).toContain(
    ...
    const lastBundleCopy = body.lastIndexOf(
      '"$bundle/feature-installer.version"',
    );
    const manifest = body.indexOf("printf 'version %s\\n'");
    const activeUnits = body.indexOf(
    ...
    const validation = body.indexOf(
      "feature-installer --validate-installation",
    );
    expect(manifest).toBeGreaterThan(lastBundleCopy);
    expect(activeUnits).toBeGreaterThan(manifest);
    expect(reload).toBeGreaterThan(activeUnits);
    expect(validation).toBeGreaterThan(reload);
    expect(body).toContain(
      "sudo /usr/lib/home-worker/feature-installer --migrate-live-view-settings",
    );
    expect(body).toContain(
      "sudo /usr/lib/home-worker/feature-installer --reset-live-view-settings",
    );
    expect(body).not.toMatch(/sudo\s+python3\s+"?\$INSTALL_DIR\/scripts\//);
  });
  ...
  it("keeps the copy calls, the manifest loop, and the helper bundle map in agreement", () => {
    ...
    expect(
      unitLoop,
      "the systemd unit copy loop no longer has the expected shape",
    ).not.toBeNull();
    ...
    const manifested = [...manifestLoop.matchAll(/"\$bundle\/([^"]+)"/g)].map(
      ([, path]) => BUNDLE + path,
    );
    const declared = new Map<string, number>(
      [
        ...helper.matchAll(
          /^ +'(\/usr\/lib\/home-worker\/[^']+)': (0o\d+),$/gm,
        ),
  ...
  it("grants only the four fixed no-block feature starts through the new sudoers file", () => {
    ...
    expect(match).toContain(
      "/bin/systemctl start --no-block homeworker-feature-supervisor-restart.service",
    );
    expect(match).toContain(
      "/bin/systemctl start --no-block homeworker-feature-host-reboot.service",
    );
    expect(match).toContain(
      "/bin/systemctl start --no-block homeworker-live-view-policy-apply.service",
    );
  ...
  it("keeps an unprivileged updater fail-closed and free of root artifact installation", () => {
    ...
    expect(update).not.toMatch(
      /sudo\s+(?:install|mv|chown|systemctl\s+daemon-reload)/,
    );
  });
  ...
  it("uses the fixed worker config while loading root-owned RTSP templates", () => {
    ...
    expect(routines).not.toContain(
      "$INSTALL_DIR/systemd/homeworker-stream-net.service",
    );
  });
});
