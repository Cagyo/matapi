...
| `ArchiveRegistrationLookupPort` (`ARCHIVE_REGISTRATION_LOOKUP`) | `FindRegisteredArchiveArtifactUseCase` | ✅ provider-neutral Camera→Archive lookup of one immutable source fingerprint; the token aliases the same use-case singleton and never exposes persistence. | [archive-registration-lookup.port.ts](../src/archive/application/ports/archive-registration-lookup.port.ts) |
...
| `CompletedMotionVideoPort` (`COMPLETED_MOTION_VIDEO`) | `FsCompletedMotionVideoAdapter` | ✅ exact-root, no-follow Motion video validation with 60-second stability and process-local `opendir`/streaming-hash state before archive registration. | [completed-motion-video.port.ts](../src/camera/domain/ports/completed-motion-video.port.ts) |
