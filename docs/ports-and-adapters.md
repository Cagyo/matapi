...
| Port | Adapters | Status | Source |
|---|---|---|---|
| `BotGateway` | `GrammyBotGateway` | ✅ single intentional gateway; do not abstract grammY itself further. It installs the Home callback acknowledgement middleware and then grammY `sequentialize` constraints for private chat and user before locale resolution/handlers. | [grammy-bot.gateway.ts](../src/telegram/infrastructure/grammy-bot.gateway.ts) |
...
| `HomeActionRepositoryPort` (`HOME_ACTION_REPOSITORY`) | `DrizzleHomeActionRepository` (real), `InMemoryHomeActionRepository` (mock/tests) | ✅ canonical — one bounded current receipt per `(userId, chatId, kind)`; external cleanup/restart claims transition atomically from pending to executing, then to a terminal status. | [home-action-repository.port.ts](../src/telegram/application/ports/home-action-repository.port.ts) |
...
| `HomeMessageDeliveryPort` (`HOME_MESSAGE_DELIVERY`) | `TelegramHomeMessageAdapter` (real), `InMemoryHomeMessageDeliveryAdapter` (mock/tests) | ✅ canonical — owns Home send, edit, best-effort keyboard stripping, and Close copy. `BOT_MODE=mock` selects the in-memory adapter; real mode renders through grammY. | [home-message-delivery.port.ts](../src/telegram/application/ports/home-message-delivery.port.ts) |
