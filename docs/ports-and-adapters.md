...
| `OtaOperationLauncherPort` (`OTA_OPERATION_LAUNCHER`) | `FlockOtaOperationLauncherAdapter` | ✅ exact checked-identity request persisted durably before fixed detached `flock`; returns started only after a strict fd-3 journal receipt and EOF. | [ota-operation-launcher.port.ts](../src/system/domain/ports/ota-operation-launcher.port.ts) |
...
| `OtaAdminNotificationPort` (`OTA_ADMIN_NOTIFICATIONS`) | `OtaAdminNotificationService` (register/clear seam) ← `TelegramOtaAdminNotificationAdapter` registered by `GrammyBotGateway` | ✅ typed, system-owned release/failure notices only; Telegram owns admin selection, per-user locale rendering, and direct delivery. A delivery count acknowledges the durable allowance only after at least one successful send. | [ota-admin-notification.port.ts](../src/system/domain/ports/ota-admin-notification.port.ts) |
...
| `UpdateDiscoveryOptions` (`UPDATE_DISCOVERY_OPTIONS`) | validated config loaded once at the `SystemModule` OTA factory boundary | ✅ real mode requires the canonical stable HTTPS feed, architecture-compatible target, and explicit active-key trust base; stub/test mode remains explicit. Defaults to a 60-minute cadence and five-minute jitter ceiling. | [update-discovery-options.port.ts](../src/system/application/ports/update-discovery-options.port.ts) |
