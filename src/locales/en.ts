...
const enCatalog = {
  ...
  liveViewSettings: {
    title: 'Live view setup', enabled: 'Enabled', disabled: 'Disabled',
    configured: (state: string) => `Configured: ${state}`,
    active: (state: string) => `Active: ${state}`,
    generations: (configured: number, active: number | null) => `Configured generation: ${configured}; active generation: ${active ?? '—'}`,
    restartRequired: 'Restart required', dependenciesReady: 'Dependencies ready', dependenciesUnavailable: 'Dependencies unavailable',
    cloudflared: 'Cloudflared', rtsp: 'RTSP runtime', none: 'None configured',
    networkNote: 'RTSP requires allowed networks. Motion MJPEG does not.',
    rtspNeedsNetwork: 'RTSP requires at least one allowed network.',
    enable: 'Enable', disable: 'Disable', networks: 'Allowed networks', manual: 'Enter CIDR manually',
    manualPrompt: 'Enter a private IPv4 or IPv6 network in CIDR notation.',
    review: 'Review changes', save: 'Save & restart', cancel: 'Cancel',
    reviewWarning: 'Current live viewing will end and the worker will restart.',
    remove: (cidr: string) => `Remove ${cidr}`,
    previous: 'Previous', next: 'Next', moreNetworks: 'More networks available — use manual entry for networks not shown.',
    normalization: (entered: string, canonical: string) => `Entered: ${entered}\nCanonical network: ${canonical}\nConfirm before adding this network.`,
    confirmNormalized: 'Add canonical network', invalid: 'Invalid private network CIDR. Check the entry and try again.',
    limit: 'At most 16 networks are allowed. Remove an entry first.', duplicate: 'This network is already in the draft.',
    startAgain: 'This setup expired or is no longer current. Start again from Live view setup.',
    busy: 'Another live view change is already running. Try again when it finishes.',
    legacyAttention: 'Legacy live view settings were not imported. Review and save the settings.',
    repairRequired: 'Live view configuration repair required.',
    applying: 'Applying live view settings…', savedRestarting: 'Saved — restarting', savedRestartRequired: 'Saved — restart required',
    retryRestart: 'Retry restart', success: 'Live view settings saved and activated.',
    failure: (reason: string) => `Live view settings could not be applied: ${reason}`,
    failures: {
      'request-invalid': 'The settings request was invalid.', 'stale-generation': 'Settings changed. Start again.',
      'settings-state-unsafe': 'Configuration repair required.', 'policy-apply-failed': 'The network policy could not be applied.',
      'service-unhealthy': 'The live view service is unhealthy.', 'rtsp-assets-absent': 'RTSP runtime assets are missing.',
      'interrupted': 'The change was interrupted. Start again.', 'helper-version-mismatch': 'The live view helper needs an update.',
      'live-work-not-quiescent': 'Live viewing could not be stopped safely. Try again.',
      'request-publish-failed': 'The settings request could not be published.', 'unit-start-failed': 'The settings helper could not start.',
      'restart-dispatch-failed': 'Restart could not be dispatched. Retry restart.',
      'restart-activation-timeout': 'Restart did not complete. Retry restart.', 'dependency-unready': 'Live view dependencies are unavailable.',
    },
  },
  feature: {
