...
const enCatalog = {
  ...
  feature: {
    ...
    confirmation: { install: (name: string, scope: string) => `Install ${name} — ${scope}`, enable: (name: string, scope: string) => `Enable ${name} — ${scope}`, disable: (name: string, scope: string) => `Disable ${name} — ${scope}`, verify: (name: string, _scope: string) => `Verify ${name}` },
