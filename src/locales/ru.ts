...
const ruCatalog = {
  ...
  liveViewSettings: {
    title: 'Настройка прямого эфира', enabled: 'Включено', disabled: 'Выключено',
    configured: (state: string) => `Настроено: ${state}`,
    active: (state: string) => `Активно: ${state}`,
    generations: (configured: number, active: number | null) => `Версия настроек: ${configured}; активная версия: ${active ?? '—'}`,
    restartRequired: 'Требуется перезапуск', dependenciesReady: 'Зависимости готовы', dependenciesUnavailable: 'Зависимости недоступны',
    cloudflared: 'Cloudflared', rtsp: 'Среда RTSP', none: 'Не настроены',
    networkNote: 'Для RTSP нужны разрешённые сети. Для Motion MJPEG они не нужны.',
    rtspNeedsNetwork: 'Для RTSP нужна хотя бы одна разрешённая сеть.',
    enable: 'Включить', disable: 'Выключить', networks: 'Разрешённые сети', manual: 'Ввести CIDR вручную',
    manualPrompt: 'Введите частную сеть IPv4 или IPv6 в формате CIDR.',
    review: 'Проверить изменения', save: 'Сохранить и перезапустить', cancel: 'Отмена',
    reviewWarning: 'Текущий просмотр завершится, и рабочий процесс будет перезапущен.',
    remove: (cidr: string) => `Удалить ${cidr}`,
    previous: 'Назад', next: 'Далее', moreNetworks: 'Есть другие сети — введите вручную сеть, которой нет в списке.',
    normalization: (entered: string, canonical: string) => `Введено: ${entered}\nАдрес сети: ${canonical}\nПодтвердите добавление этой сети.`,
    confirmNormalized: 'Добавить адрес сети', invalid: 'Некорректный CIDR частной сети. Проверьте ввод и повторите.',
    limit: 'Допускается не более 16 сетей. Сначала удалите одну из записей.', duplicate: 'Эта сеть уже добавлена.',
    startAgain: 'Настройка истекла или уже не актуальна. Начните заново в настройках прямого эфира.',
    busy: 'Другое изменение прямого эфира уже выполняется. Повторите после его завершения.',
    legacyAttention: 'Прежние настройки прямого эфира не импортированы. Проверьте и сохраните настройки.',
    repairRequired: 'Требуется восстановить настройки прямого эфира.',
    applying: 'Применяем настройки прямого эфира…', savedRestarting: 'Сохранено — перезапуск', savedRestartRequired: 'Сохранено — требуется перезапуск',
    retryRestart: 'Повторить перезапуск', success: 'Настройки прямого эфира сохранены и активированы.',
    failure: (reason: string) => `Не удалось применить настройки прямого эфира: ${reason}`,
    failures: {
      'request-invalid': 'Некорректный запрос настроек.', 'stale-generation': 'Настройки изменились. Начните заново.',
      'settings-state-unsafe': 'Требуется восстановить настройки.', 'policy-apply-failed': 'Не удалось применить сетевую политику.',
      'service-unhealthy': 'Служба прямого эфира неисправна.', 'rtsp-assets-absent': 'Отсутствуют компоненты среды RTSP.',
      'interrupted': 'Изменение прервано. Начните заново.', 'helper-version-mismatch': 'Нужно обновить помощник прямого эфира.',
      'live-work-not-quiescent': 'Не удалось безопасно остановить просмотр. Повторите попытку.',
      'request-publish-failed': 'Не удалось опубликовать запрос настроек.', 'unit-start-failed': 'Не удалось запустить помощник настроек.',
      'restart-dispatch-failed': 'Не удалось отправить команду перезапуска. Повторите перезапуск.',
      'restart-activation-timeout': 'Перезапуск не завершён. Повторите перезапуск.', 'dependency-unready': 'Зависимости прямого эфира недоступны.',
    },
  },
  feature: {
