...
const ruCatalog = {
  ...
  feature: {
    ...
    detail: ({ name, state, dependencies, controls, monitoring, downtime, attention }: { name: string; state: string; dependencies: string; controls: string; monitoring: string; downtime: string; attention: string | null }) => [name, `Состояние: ${state}`, `Зависимости: ${dependencies}`, `Управляет: ${controls}`, `Мониторинг: ${monitoring}`, downtime, attention ? `Внимание: ${attention}` : ''].filter(Boolean).join('\n'),
