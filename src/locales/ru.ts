...
const ruCatalog = {
  ...
  feature: {
    ...
    confirmation: { install: (name: string, scope: string) => `Установить ${name} — ${scope}`, enable: (name: string, scope: string) => `Включить ${name} — ${scope}`, disable: (name: string, scope: string) => `Выключить ${name} — ${scope}`, verify: (name: string, _scope: string) => `Проверить ${name}` },
    restartScope: { worker: 'перезапуск worker', supervisor: 'перезапуск службы', host: 'перезагрузка Pi' },
    failure: { 'request-invalid': 'некорректный запрос установки', 'request-publish-failed': 'не удалось поставить запрос в очередь', 'dependency-install-failed': 'не удалось установить зависимости', 'privileged-verification-failed': 'не пройдена привилегированная проверка', 'application-verification-failed': 'не пройдена проверка приложения', 'partial-state-uncertain': 'состояние требует восстановления', 'helper-version-mismatch': 'нужно обновить установщик', 'result-invalid': 'некорректный результат установщика', interrupted: 'установка прервана' },
  ...
  system: {
    online: (v: SystemOnlineView): string => {
      ...
      if (!v.archiveRecovered) {
        lines.push('⚠️ Не удалось восстановить работу архива — выгрузка видео и резервные копии приостановлены до следующего перезапуска.');
      }
