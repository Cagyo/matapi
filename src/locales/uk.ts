...
const ukCatalog = {
  ...
  feature: {
    ...
    confirmation: { install: (name: string, scope: string) => `Встановити ${name} — ${scope}`, enable: (name: string, scope: string) => `Увімкнути ${name} — ${scope}`, disable: (name: string, scope: string) => `Вимкнути ${name} — ${scope}`, verify: (name: string, _scope: string) => `Перевірити ${name}` },
    ...
    errors: { installStart: (name: string) => `❌ Не вдалося поставити ${name} у чергу.`, notInstalled: (name: string) => `❌ ${name} не встановлена.`, inconsistent: (name: string) => `❌ ${name} потребує відновлення перед зміною.`, alreadyEnabled: (name: string) => `${name} уже увімкнена.`, alreadyDisabled: (name: string) => `${name} уже вимкнена.`, restartFailed: (name: string, scope: string) => `❌ ${name} змінена, але не вдалося запустити: ${scope}.` },
