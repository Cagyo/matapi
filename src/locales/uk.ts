...
const ukCatalog = {
  ...
  feature: {
    ...
    failure: { 'request-invalid': 'некоректний запит встановлення', 'request-publish-failed': 'не вдалося поставити запит у чергу', 'dependency-install-failed': 'не вдалося встановити залежності', 'privileged-verification-failed': 'не пройдено привілейовану перевірку', 'application-verification-failed': 'не пройдено перевірку застосунку', 'partial-state-uncertain': 'стан потребує відновлення', 'helper-version-mismatch': 'потрібно оновити встановлювач', 'result-invalid': 'некоректний результат встановлювача', interrupted: 'встановлення перервано' },
  ...
  camera: {
    ...
    adminAlert: {
      ...
      motionScanFailing: (code: string) =>
        `⚠️ Обхід відео Motion продовжує завершуватися помилкою (${code}). Нові записи не архівуються, доки проблему не буде усунено.`,
      diskWarning:
