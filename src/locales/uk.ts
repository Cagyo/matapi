...
export const commands: CommandDescriptor[] = [
  ...
  },
  {
    ...
    usage: '/feature list|install|enable|disable <назва> — керувати додатковими можливостями',
    scope: 'admin',
  },
  {
    command: 'update',
  ...
  },
  {
    ...
    usage: '/system_update — оновити дозволені пакети ОС (оновлення Node виконуються вручну)',
    scope: 'admin',
  },
  {
    command: 'restart',
  ...
  {
    command: 'gdrive',
    description: 'Підключити, перевірити, повторити або відключити Google Drive',
    usage: '/gdrive connect|status|retry|disconnect — керування Google Drive',
    scope: 'admin',
  },
  {
    command: 'settings',
...
const ukCatalog = {
  ...
  liveViewSettings: {
    title: 'Налаштування прямого ефіру', enabled: 'Увімкнено', disabled: 'Вимкнено',
    configured: (state: string) => `Налаштовано: ${state}`,
    active: (state: string) => `Активно: ${state}`,
    generations: (configured: number, active: number | null) => `Версія налаштувань: ${configured}; активна версія: ${active ?? '—'}`,
    restartRequired: 'Потрібен перезапуск', dependenciesReady: 'Залежності готові', dependenciesUnavailable: 'Залежності недоступні',
    cloudflared: 'Cloudflared', rtsp: 'Середовище RTSP', none: 'Не налаштовано',
    networkNote: 'Для RTSP потрібні дозволені мережі. Для Motion MJPEG вони не потрібні.',
    rtspNeedsNetwork: 'Для RTSP потрібна хоча б одна дозволена мережа.',
    enable: 'Увімкнути', disable: 'Вимкнути', networks: 'Дозволені мережі', manual: 'Ввести CIDR вручну',
    manualPrompt: 'Введіть приватну мережу IPv4 або IPv6 у форматі CIDR.',
    review: 'Перевірити зміни', save: 'Зберегти й перезапустити', cancel: 'Скасувати',
    reviewWarning: 'Поточний перегляд завершиться, і робочий процес буде перезапущено.',
    remove: (cidr: string) => `Видалити ${cidr}`,
    previous: 'Назад', next: 'Далі', moreNetworks: 'Є інші мережі — введіть вручну мережу, якої немає у списку.',
    normalization: (entered: string, canonical: string) => `Введено: ${entered}\nАдреса мережі: ${canonical}\nПідтвердьте додавання цієї мережі.`,
    confirmNormalized: 'Додати адресу мережі', invalid: 'Некоректний CIDR приватної мережі. Перевірте введення та повторіть.',
    limit: 'Дозволено не більше 16 мереж. Спочатку видаліть один із записів.', duplicate: 'Цю мережу вже додано.',
    startAgain: 'Налаштування спливло або вже не актуальне. Почніть знову в налаштуваннях прямого ефіру.',
    busy: 'Інша зміна прямого ефіру вже виконується. Повторіть після її завершення.',
    legacyAttention: 'Попередні налаштування прямого ефіру не імпортовано. Перевірте та збережіть налаштування.',
    repairRequired: 'Потрібно відновити налаштування прямого ефіру.',
    applying: 'Застосовуємо налаштування прямого ефіру…', savedRestarting: 'Збережено — перезапуск', savedRestartRequired: 'Збережено — потрібен перезапуск',
    retryRestart: 'Повторити перезапуск', success: 'Налаштування прямого ефіру збережено та активовано.',
    failure: (reason: string) => `Не вдалося застосувати налаштування прямого ефіру: ${reason}`,
    failures: {
      'request-invalid': 'Некоректний запит налаштувань.', 'stale-generation': 'Налаштування змінилися. Почніть знову.',
      'settings-state-unsafe': 'Потрібно відновити налаштування.', 'policy-apply-failed': 'Не вдалося застосувати мережеву політику.',
      'service-unhealthy': 'Служба прямого ефіру несправна.', 'rtsp-assets-absent': 'Відсутні компоненти середовища RTSP.',
      'interrupted': 'Зміну перервано. Почніть знову.', 'helper-version-mismatch': 'Потрібно оновити помічник прямого ефіру.',
      'live-work-not-quiescent': 'Не вдалося безпечно зупинити перегляд. Повторіть спробу.',
      'request-publish-failed': 'Не вдалося опублікувати запит налаштувань.', 'unit-start-failed': 'Не вдалося запустити помічник налаштувань.',
      'restart-dispatch-failed': 'Не вдалося надіслати команду перезапуску. Повторіть перезапуск.',
      'restart-activation-timeout': 'Перезапуск не завершено. Повторіть перезапуск.', 'dependency-unready': 'Залежності прямого ефіру недоступні.',
    },
  },
  feature: {
    names: { digital: 'Цифрові входи', uart: 'Датчик CO₂ UART', zigbee: 'Zigbee', motion: 'Камера Motion', rtsp: 'Камера RTSP' },
    description: { digital: 'підтримка цифрових входів GPIO', uart: 'підтримка датчика CO₂ UART', zigbee: 'підтримка моста Zigbee MQTT', motion: 'запис камерою Motion', rtsp: 'середовище прямої трансляції RTSP' },
    ...
    confirmation: { install: (name: string, scope: string) => `Встановити ${name} — ${scope}`, reinstall: (name: string, scope: string) => `Перевстановити ${name} — ${scope}`, enable: (name: string, scope: string) => `Увімкнути ${name} — ${scope}`, disable: (name: string, scope: string) => `Вимкнути ${name} — ${scope}`, verify: (name: string, _scope: string) => `Перевірити ${name}` },
    restartScope: { worker: 'перезапуск worker', supervisor: 'перезапуск служби', host: 'перезавантаження Pi' },
    failure: { 'request-invalid': 'некоректний запит встановлення', 'request-publish-failed': 'не вдалося поставити запит у чергу', 'local-network-unavailable': 'немає придатної локальної мережі', 'network-policy-generation-failed': 'не вдалося підготувати мережеву політику', 'dependency-install-failed': 'не вдалося встановити залежності', 'privileged-verification-failed': 'не пройдено привілейовану перевірку', 'application-verification-failed': 'не пройдено перевірку застосунку', 'partial-state-uncertain': 'стан потребує відновлення', 'helper-version-mismatch': 'потрібно оновити встановлювач', 'result-invalid': 'некоректний результат встановлювача', interrupted: 'встановлення перервано' },
    preRestart: (name: string, scope: string) => `⏳ ${name} готова. Запускається ${scope}; остаточний результат буде після відновлення.`,
    progress: { installing: (name: string) => `⏳ Встановлюється ${name}. Результат буде надіслано пізніше.`, reinstalling: (name: string) => `⏳ ${name} перевстановлюється в поточній мережі. Результат буде надіслано пізніше.` },
    outcome: { success: (name: string) => `✅ ${name}: виконано.`, failure: (name: string, reason: string) => `❌ ${name}: помилка (${reason}).`, genericFailure: (name: string) => `❌ Не вдалося завершити ${name}.`, recoveredSuccess: (name: string) => `Відновлено успішний результат для ${name}.`, recoveredFailure: (name: string, reason: string) => `Відновлено помилковий результат для ${name} (${reason}).` },
    ...
    errors: { installStart: (name: string) => `❌ Не вдалося поставити ${name} у чергу.`, notInstalled: (name: string) => `❌ ${name} не встановлена.`, inconsistent: (name: string) => `❌ ${name} потребує відновлення перед зміною.`, alreadyEnabled: (name: string) => `${name} уже увімкнена.`, alreadyDisabled: (name: string) => `${name} уже вимкнена.`, restartFailed: (name: string, scope: string) => `❌ ${name} змінена, але не вдалося запустити: ${scope}.`, reinstallUnavailable: (name: string) => `❌ Зараз ${name} не можна перевстановити.` },
  ...
  menu: {
    ...
    usage: {
      ...
      feature: 'Використання: /feature list|install|enable|disable <назва> — керування додатковими можливостями',
  ...
  camera: {
    ...
    /*
     * RTSP-джерела: обов'язковий розділ без відкату на англійську. Ключі
     * збігаються з `en.camera.sources` — див. `LocaleCatalog` і тести паритету.
     */
    sources: {
      dashboardButton: '📡 RTSP-джерела',

      overview: {
        title: '📡 RTSP-джерела камер',
        page: (page: number, pageCount: number) => `Сторінка ${page} з ${pageCount}`,
        previous: '‹ Назад',
        next: 'Далі ›',
        addCamera: '➕ Додати RTSP-камеру',
      },
      policy: {
        scope: 'Лише локальна мережа',
        network: (network: { interface: string; cidr: string; family: 4 | 6 }) =>
          `• ${network.interface} · ${network.cidr} (IPv${network.family})`,
        noNetworks: 'Наразі жодну мережу камер не описано.',
        state: {
          ready: 'Камери доступні лише в мережах вище і більше ніде.',
          stale: '⚠️ Політика мережі камер більше не описує цю мережу. Перевстановіть RTSP, щоб оновити її.',
          unavailable: '⚠️ Не вдалося прочитати політику мережі камер. Перевстановіть RTSP, щоб відновити її.',
        },
      },
      emptyState: {
        title: 'RTSP-камери ще не налаштовано.',
        body: 'RTSP-камера передає потік зі своєї адреси в локальній мережі вище. Додайте камеру, щоб дивитися її наживо.',
        addFirst: '➕ Додати першу камеру',
      },
      progress: {
        testing: '⏳ Перевіряємо підключення…',
        removing: '⏳ Видаляємо джерело камери…',
      },

      statuses: {
        'configured-verified': '✅ Готово',
        'credentials-required': '🔑 Потрібна адреса',
        'not-ready': '⏳ Не готово',
        'needs-attention': '⚠️ Потребує уваги',
      },
      relationships: {
        allowed: 'усередині мережі камер',
        blocked: 'поза мережею камер',
        unresolved: 'за адресою, яку не вдається розв’язати',
      },
      ...
      detail: (input: {
        ...
        `Адреса: ${input.host}`,
        `Статус: ${input.status}`,
        `Мережа: ${input.relationship}`,
      ].join('\n'),
      reverificationDue: '⚠️ Не перевірено за чинною політикою мережі камер. Перевірте підключення.',
      detailButtons: {
        test: '🧪 Перевірити підключення',
        changeAddress: '🔗 Змінити адресу',
        details: 'ℹ️ Подробиці',
      },
      details: {
        title: 'ℹ️ Параметри підключення',
        body: (input: {
          security: string;
          transport: string;
          profile: string;
          relationship: string;
        }) => [input.security, input.transport, input.profile, `Мережа: ${input.relationship}`].join('\n'),
        security: {
          none: 'Захист: звичайний RTSP, без шифрування',
          strict: 'Захист: RTSPS, сертифікат перевіряється',
        },
        transports: {
          auto: 'Транспорт: обирається автоматично',
          tcp: 'Транспорт: TCP',
          udp: 'Транспорт: UDP',
        },
        profiles: {
          eco: 'Якість: економна',
          balanced: 'Якість: збалансована',
          quality: 'Якість: висока',
        },
      },

      add: {
        title: 'Додати RTSP-камеру',
        choose: 'Створіть нову камеру або підключіть джерело до вже наявної.',
        create: '➕ Створити RTSP-камеру',
        attach: '🔗 Підключити до наявної камери',
        chooseCamera: 'Виберіть камеру, якій належить це джерело:',
      },

      prompts: {
        name: 'Дайте відповідь на це повідомлення назвою камери.',
        nameHint: 'До 64 символів і відмінна від назв усіх уже доданих камер.',
        credential: 'Дайте відповідь на це повідомлення адресою камери.',
        replyHint: 'Дайте відповідь саме на це повідомлення. Окреме нове повідомлення прочитано не буде.',
        invalidName: 'Таку назву використати не можна. Дайте відповідь ще раз звичайним текстом, до 64 символів.',
        expired: (minutes: number) =>
          `⏳ Це налаштування камери минуло через ${minutes} ${plural(minutes, 'хвилину', 'хвилини', 'хвилин')}. Відкрийте RTSP-джерела, щоб почати заново.`,
        cancelled: 'Налаштування камери скасовано. Нічого не змінено.',
        cancelButton: '❌ Скасувати',
      },

      privacyNotice: (input: { networks: string; minutes: number }) => [
        '🔒 Перш ніж надіслати адресу камери',
        'Приймаються лише адреси RTSP і суворого RTSPS.',
        `Камери доступні в мережах:\n${input.networks}`,
        'Адреса може містити ім’я користувача та пароль.',
        'У Telegram немає секретного каналу — ваша відповідь надійде звичайним повідомленням у цей чат.',
        'Воно видаляється одразу, але видалення виконується за можливості й може не спрацювати.',
        `Термін дії запиту — ${input.minutes} ${plural(input.minutes, 'хвилина', 'хвилини', 'хвилин')}.`,
      ].join('\n\n'),

      removal: {
        confirmCamera: (cameraName: string) =>
          `Видалити RTSP-камеру ${cameraName}?\nКамеру та збережену адресу буде видалено. Записані події збережуться.`,
        confirmSource: (cameraName: string) =>
          `Видалити RTSP-джерело в камери ${cameraName}?\nКамера залишиться; буде видалено лише її адресу та збережені облікові дані.`,
        removeCameraButton: '🗑 Видалити RTSP-камеру',
        removeSourceButton: '🗑 Видалити RTSP-джерело',
        keep: '« Залишити',
        removedCamera: (cameraName: string) => `✅ RTSP-камеру ${cameraName} видалено.`,
        removedSource: (cameraName: string) => `✅ RTSP-джерело видалено в камери ${cameraName}.`,
      },

      outcomes: {
        created: (cameraName: string) => `✅ Камеру ${cameraName} налаштовано, і вона відповідає.`,
        attached: (cameraName: string) => `✅ RTSP-джерело підключено до камери ${cameraName}, і воно відповідає.`,
        replaced: (cameraName: string) => `✅ Адресу камери ${cameraName} замінено, і вона відповіла.`,
        tested: (cameraName: string) => `✅ Камера ${cameraName} відповіла за збереженою адресою. Нічого не змінено.`,
      },

      errors: {
        'invalid-address': '❌ Цю адресу використати не можна. Потрібна адреса RTSP або суворого RTSPS із зазначенням вузла.',
        'outside-policy': '❌ Адреса камери поза дозволеними мережами камер. Камера може бути в іншій підмережі або доступна лише через IPv6, тоді як політика дозволяє IPv4 — чи навпаки.',
        'name-taken': '❌ Інша камера вже використовує цю назву. Поверніться назад і виберіть іншу.',
        'host-not-found': '❌ Ім’я вузла камери не розв’язується. Перевірте ім’я або вкажіть адресу.',
        'host-unreachable': '❌ Камера не відповіла. Перевірте, чи ввімкнена вона та чи є в цій мережі.',
        'authentication-failed': '❌ Камера відхилила запит. Перевірте ім’я користувача та пароль — а також шлях потоку: багато камер відхиляють його так само, навіть коли облікові дані правильні.',
        'tls-verification-failed': '❌ Сертифікат камери не пройшов перевірку. Перевірте ім’я вузла та центр сертифікації.',
        'unsupported-stream': '❌ Камера відповіла потоком, який цей пристрій не може відтворити. Спробуйте її додатковий потік або профіль H.264.',
        'timed-out': '❌ Камера відповідала надто довго. Перевірте мережу й спробуйте ще раз.',
        'feature-unavailable': '❌ Підтримка RTSP-камер зараз недоступна, тому нічого не змінено.',
        'policy-stale': '❌ Політика мережі камер більше не є чинною, тому нічого не змінено. Перевстановіть RTSP у поточній мережі.',
        'source-stale': '❌ Це джерело камери змінилося, поки ви з ним працювали. Нічого не змінено — відкрийте його знову, щоб побачити поточний стан.',
        'probe-failed': '❌ Не вдалося перевірити камеру. Перевірте адресу, облікові дані та мережу.',
      },
      actions: {
        retry: '↻ Повторити',
        'change-address': '🔗 Змінити адресу',
        back: '« Назад',
        'reinstall-rtsp': '🔁 Перевстановити RTSP',
      },

      startAgain: '↻ Почати заново',
      credentialDeletionFailed: (cameraName: string) =>
        `⚠️ Можливо, вашу відповідь з адресою камери ${cameraName} не було видалено. Перевірте цей чат і видаліть її самі, якщо вона ще там.`,
    ...
    adminAlert: {
      ...
      motionScanFailing: (code: string) =>
        `⚠️ Обхід відео Motion продовжує завершуватися помилкою (${code}). Нові записи не архівуються.`,
      diskWarning:
  ...
  system: {
    online: (v: SystemOnlineView): string => {
      ...
      if (!v.archiveRecovered) {
        lines.push('⚠️ Не вдалося відновити роботу архіву — завантаження відео та резервні копії призупинено до наступного перезапуску.');
      }
