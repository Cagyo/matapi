...
const ukCatalog = {
  ...
  feature: {
    ...
    detail: ({ name, state, dependencies, controls, monitoring, downtime, attention }: { name: string; state: string; dependencies: string; controls: string; monitoring: string; downtime: string; attention: string | null }) => [name, `Стан: ${state}`, `Залежності: ${dependencies}`, `Керує: ${controls}`, `Моніторинг: ${monitoring}`, downtime, attention ? `Увага: ${attention}` : ''].filter(Boolean).join('\n'),
