export type HomeActionReceipt =
  | { id: string; userId: number; chatId: number; kind: 'pause-confirmation'; sessionToken: string; status: 'pending'; expiresAt: Date; payload: { hours: 1 | 4 | 8 } }
  ...
  | { id: string; userId: number; chatId: number; kind: 'undo-non-critical-pause'; sessionToken: null; status: 'pending'; expiresAt: Date; payload: { foundationReceiptId: number; expectedRevision: number } }
  | { id: string; userId: number; chatId: number; kind: 'undo-quiet-hours'; sessionToken: null; status: 'pending'; expiresAt: Date; payload: { start: string | null; end: string | null; expectedRevision: number } };
...
export function isHomeActionReceipt(value: unknown): value is HomeActionReceipt {
  ...
  if (value.kind === 'pause-confirmation') {
    return value.status === 'pending' && typeof value.sessionToken === 'string' && hasKeys(value.payload, ['hours'])
  ...
  if (value.kind === 'undo-non-critical-pause') {
    return value.status === 'pending' && value.sessionToken === null && hasKeys(value.payload, ['foundationReceiptId', 'expectedRevision'])
      && isSafeInteger(value.payload.foundationReceiptId) && isSafeInteger(value.payload.expectedRevision);
  }
  return value.kind === 'undo-quiet-hours' && value.status === 'pending' && value.sessionToken === null
