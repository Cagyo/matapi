...
export class FsFeatureSeedConfigAdapter implements FeatureSeedConfigPort {
  constructor(
    private readonly path = resolve(process.cwd(), "features.json"),
  ) {}
...
function parseEnabled(raw: string): readonly ManageableFeatureName[] | null {
  ...
  const currentShape =
    keys.length === 2 && keys[0] === "enabled" && keys[1] === "timestamp";
  const legacyShape =
    keys.length === 3 &&
    keys[0] === "enabled" &&
    keys[1] === "liveStream" &&
  ...
  if (
    ...
    (legacyShape && typeof value.liveStream !== "boolean")
  )
    return null;
  if (
  ...
  for (const item of value.enabled) {
    ...
    seen.add(item);
    // Live view remains deferred until its typed settings authority exists.
    // The removed legacy boolean is accepted only as bounded file-shape
    // compatibility and can never seed RTSP as installed or enabled.
    if (item !== "rtsp") enabled.push(item);
  }
  return enabled;
}
