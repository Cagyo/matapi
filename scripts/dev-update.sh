...
# If running under zsh (whether sourced with '.' or executed directly),
# switch to bash since this script uses bash syntax (`#!/bin/bash`).
if [ -n "${ZSH_VERSION:-}" ]; then
  _script_path="${(%):-%x}"
  if [[ "${ZSH_EVAL_CONTEXT:-}" =~ :file$ ]]; then
    echo "Notice: dev-update.sh was sourced in zsh; launching in bash subshell..." >&2
  fi
  bash "$_script_path" "$@"
  return $? 2>/dev/null || exit $?
fi

# If sourced inside bash, re-launch as a clean subshell so traps and set -e don't affect the parent shell.
if [ -n "${BASH_SOURCE:-}" ] && [ "${BASH_SOURCE[0]:-$0}" != "${0:-}" ]; then
  echo "Notice: dev-update.sh was sourced in bash; launching in bash subshell..." >&2
  bash "${BASH_SOURCE[0]}" "$@"
  return $? 2>/dev/null || exit $?
fi
...
SSH_OPTS=(-4 -o ConnectTimeout=10 -o ControlMaster=auto -o ControlPath=/tmp/matapi-dev-update-ssh-%r@%h:%p -o ControlPersist=300)
...
on_exit() {
  ...
  if [ $exit_code -ne 0 ]; then
    echo "" >&2
    echo "❌ Error: dev-update.sh terminated with exit code $exit_code." >&2
...
sshpass -p "$REMOTE_PASS" ssh -t "${SSH_OPTS[@]}" "$REMOTE_USER@$REMOTE_HOST" "cd ~/matapi/worker && echo '$REMOTE_PASS' | sudo -S -H -u homeworker env HOME_WORKER_REPO=\"\$(pwd)\" ./scripts/update.sh"
